...
public:
	midOut()
	{
		...
		try
		{
			...
			if (chooseMidiPort(mout) == false)
			{
				cout << "Midi not connected" << endl;
			}
			else
			{
				std::cout << "\nReady for MIDI output ... press <enter> to quit.\n";
			}
		}
	...
	void send_message(vector<unsigned char> msg)
	{
...
public:
    Sample(const char * fname, int channel,bool loop=false)
    {
        ...
        {
            cerr<<"You have entered an invalid filename"<<endl;
        }
	...
	Sample(const Sample& old_sample)
	{
		filename = old_sample.filename;
		if (chan == nullptr)
		{
			chan = chans::get_chans();
		}
		...
		//find new channel
		int chan_ind = 0;
		for (; chan_ind<32; chan_ind++)
		{
			if (chan[chan_ind] == nullptr)
			{
				chan_index = chan_ind;
				break;
			}
		...
		if (chan_ind == 32)
		{
		//	cout << "ran out of channels placing on old channel" << endl;
			chan_index = old_sample.chan_index;
		}
		FMOD::Sound * snd_ref = old_sample.snd;
		FMOD_MODE mode;
		snd_ref->getMode(&mode);
  ...
  /* void stop()
    {
        ...
        {
            result=chan[chan_index]->stop();
        }
        else
        {
            cerr<<"Either the sound or the channel hasnt been properly initialized"<<endl;
        }
    } */
    void set_paused(bool pause)
    {
        if(snd!=nullptr && chan[chan_index]!=nullptr&&soundsystem!=nullptr)
        {
            result=chan[chan_index]->setPaused(pause);
        }
        else
        {
            cerr<<"Either the sound or the channel hasnt been properly initialized"<<endl;
	...
	void set_bass(float level)
	{
		...
		{
			cerr << "You have not added a bass mid treble control to this sound." << endl;
		}
	}
    ...
    void mute_flange()
    {
		...
		{
			cerr << "You have not added flange to this sound" << endl;
		}
    }
    void unmute_flange()
    {
    ...
    void mute_echo()
    {
		...
		{
			cerr << "You have not added echo to this sound" << endl;
		}
    }
    void unmute_echo()
    {
	...
	unsigned int get_length() //Returns the length of the song in milliseconds
	{
...
public:
    ...
    void start()
    {
      std::thread t1(osc_server2,std::ref(*this));
      t1.join();
    }
