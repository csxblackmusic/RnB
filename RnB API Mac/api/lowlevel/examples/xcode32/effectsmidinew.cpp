...
public:
    Sample(const char * fname, int channel,bool loop=false)
    {
        ...
        if(!loop)
            ...
            soundsystem->createSound(filename, FMOD_LOOP_NORMAL, 0, &snd);
    ...
    {
        if(abs(speed)<=100 and abs(speed)>=0)
        {
            float frequency;
            int priority;
...
Sample samp1 = Sample("/Users/djames18/Documents/FMOD_Programmers_API/api/lowlevel/examples/xcode32/Verse1.wav",1,true);
Sample samp3 = Sample("/Users/djames18/Documents/FMOD_Programmers_API/api/lowlevel/examples/xcode32/Shaker2.wav",3);
const int PORT_NUM = 8000;
...
void process_gyro(const string path,vector<float> data)
{
    ...
    samp1.set_speed(abs((data.at(2)/720 )-1));
    ...
    if(abs(data.at(1))>0 && abs(data.at(1))<2)
    {
        triggered = false;
    }
}
...
void osc_server2(osc_rec &osc_obj)
{
    ...
    if (!sock.isOk())
    {
        cout << "Error opening port " << PORT_NUM << ": " << sock.errorMessage() << "\n";
    }
...
void oldmain()//int main()
{
   // midIn device = midIn(mycallback);
    //midIn device2 = midIn(othercallback);
    
    
    cout <<"executed constructor"<<endl;
    Sample samp2=Sample("/Users/djames18/Documents/FMOD_Programmers_API/api/lowlevel/examples/xcode32/Verse2.wav",4);
    cout <<"executed constructor"<<endl;
  ...
  //  samp1.set_volume(0.9);
    samp2.set_volume(0.01);
    //samp1.add_echo();
    FMOD::Channel* ch1 =samp1.get_channel();
    bool running= true;
    cout<<"entering engine loop"<<endl;
    
   //osc_rec osc_receive = osc_rec(8000,"/muse/gyro",process_gyro);
   std::thread turntable(osc_server);
    while(running)
    {
        samp1.upd();
        usleep(50);
