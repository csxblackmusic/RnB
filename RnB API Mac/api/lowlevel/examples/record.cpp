...
int FMOD_Main()
{
    ...
    */
    ...
    result = system->init(100, FMOD_INIT_NORMAL, extraDriverData); //100 is the number of channels
    ERRCHECK(result);

    int numDrivers = 0;
    result = system->getRecordNumDrivers(NULL, &numDrivers); //number of audio drivers
    ERRCHECK(result);
    ...
    */
    ...
    result = system->getRecordDriverInfo(DEVICE_INDEX, NULL, 0, NULL, &nativeRate, NULL, &nativeChannels, NULL);//second parameter can get the name of the device
                               // native rate is the native sample rate of the audio driver
    ERRCHECK(result);
    ...
    */
    ...
    exinfo.format           = FMOD_SOUND_FORMAT_PCM16; // 16 bit pcm
    exinfo.defaultfrequency = nativeRate;
    exinfo.length           = nativeRate * sizeof(short) * nativeChannels; /* 1 second buffer, size here doesn't change latency */
    
    FMOD::Sound *sound = NULL;
    result = system->createSound(0, FMOD_LOOP_NORMAL | FMOD_OPENUSER, &exinfo, &sound);//OPEN USER declares a user created sound which
    ERRCHECK(result);                                                                   // will be user created

    result = system->recordStart(DEVICE_INDEX, sound, true);    //records from device 0 into sound , true loops back to the beginning
    ERRCHECK(result);                                           //of the sound if you reach then end
    ...
    */
    do
    {
        ...
        */
        ...
        unsigned int recordDelta = (recordPos >= lastRecordPos) ? (recordPos - lastRecordPos) : (recordPos + soundLength - lastRecordPos); //TERNARY (CONDITION)? VALUE_IF_TRUE:VALUE_IF_FALSE
